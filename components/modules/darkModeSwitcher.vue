<template>
<!-- darkmode switcher -->
<span  @click="toggle" class="group relative flex h-9 w-9 rounded-full transition duration-500 hover:bg-blue-950/5 dark:hover:bg-white/10">
    <svg v-if="colorMode.value == 'light'" viewBox="0 0 24 24" fill="none" class="w-6 h-6 cursor-pointer m-auto fill-gray-500 group-hover:fill-gray-950 dark:fill-white/70 dark:group-hover:fill-white">
    ...
    <svg v-else-if="colorMode.value == 'dark'" viewBox="0 0 24 24" fill="none" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-6 h-6 cursor-pointer m-auto">
